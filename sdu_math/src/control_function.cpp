...
/////// PID function /////////////////////////////////////////////////////////////////////////
PID_function::PID_function(double dt, double max, double min, double kp, double kd, double ki, double threshold_max, double threshold_min, double time):
dt_(dt),
max_(max),
min_(min),
kp_(kp),
...
time_(time)
{
	...
	desired_values(0,7) = time_;
	desired_values(1,7) = time_;
	desired_values(2,7) = time_;
	...
	gain_traj->cal_end_point_tra_px->set_current_pose(0);
	gain_traj->cal_end_point_tra_py->set_current_pose(0);
	gain_traj->cal_end_point_tra_pz->set_current_pose(0);
...
void PID_function::set_smooth_gain_time(double time)
{
	desired_values(0,7) = time;
	desired_values(1,7) = time;
	desired_values(2,7) = time;
}
